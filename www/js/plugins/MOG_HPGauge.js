...
/*:
 * @plugindesc (v1.3) Apresenta um medidor de HP do alvo.
 * @author Moghunter
 *
 ...
 * HISTÓRICO
 * =============================================================================
 * (v1.3) - Compatibilidade com MOG Consecutive Battles.
 * (v1.2) - Correção de apresentar o HP dos aliados no modo batalha frontal.
 *        - Melhoria na codificação e na compatibilidade de plugins.
 */
...
Moghunter.hpgauge_actors = String(
  Moghunter.parameters["Gauge For Actors"] || "true"
);
Moghunter.hpgauge_enemies = String(
  Moghunter.parameters["Gauge For Enemies"] || "true"
);
Moghunter.hpgauge_selection = String(
  Moghunter.parameters["Show HP On Selection"] || "true"
);
Moghunter.hpgauge_slide = String(
  Moghunter.parameters["Slide Animation"] || "true"
);
Moghunter.hpgauge_showNumberActor = String(
  Moghunter.parameters["Show HP Number Actor"] || "false"
);
Moghunter.hpgauge_showNumberEnemy = String(
  Moghunter.parameters["Show HP Number Enemy"] || "true"
);
Moghunter.hpgauge_NumberAfter = String(
  Moghunter.parameters["Required to Kill"] || "false"
);
Moghunter.hpgauge_a_x = Number(
  Moghunter.parameters["Enemy Layout X-Axis"] || 0
);
Moghunter.hpgauge_a_y = Number(
  Moghunter.parameters["Enemy Layout Y-Axis"] || 0
);
Moghunter.hpgauge_b_x = Number(Moghunter.parameters["Enemy Meter X-Axis"] || 8);
Moghunter.hpgauge_b_y = Number(
  Moghunter.parameters["Enemy Meter Y-Axis"] || 10
);
Moghunter.hpgauge_c_x = Number(
  Moghunter.parameters["Enemy HP Number X-Axis"] || -30
);
Moghunter.hpgauge_c_y = Number(
  Moghunter.parameters["Enemy HP Number Y-Axis"] || -19
);
Moghunter.hpgauge_a_xa = Number(
  Moghunter.parameters["Actor Layout X-Axis"] || 0
);
Moghunter.hpgauge_a_ya = Number(
  Moghunter.parameters["Actor Layout Y-Axis"] || 0
);
Moghunter.hpgauge_b_xa = Number(
  Moghunter.parameters["Actor Meter X-Axis"] || 0
);
Moghunter.hpgauge_b_ya = Number(
  Moghunter.parameters["Actor Meter Y-Axis"] || 3
);
Moghunter.hpgauge_c_xa = Number(
  Moghunter.parameters["Actor HP Number X-Axis"] || -25
);
Moghunter.hpgauge_c_ya = Number(
  Moghunter.parameters["Actor HP Number Y-Axis"] || -19
);
Moghunter.hpgauge_duration = Number(
  Moghunter.parameters["Fade Duration"] || 90
);
...
Spriteset_Battle.prototype.update = function () {
  ...
  if (Imported.MOG_BattleCamera && this._hpField) {
    this.updateHPField();
  }
};
...
HPGaugeSprite.prototype.initialize = function (sprite, type) {
  ...
  this._selectionHP =
    String(Moghunter.hpgauge_selection) === "true" ? true : false;
  if (type === 0) {
    this._showHPNumber =
      String(Moghunter.hpgauge_showNumberActor) === "true" ? true : false;
  } else {
    this._showHPNumber =
      String(Moghunter.hpgauge_showNumberEnemy) === "true" ? true : false;
  }
  this._IDA = type === 0 ? true : this.enemyIDA();
  if (type === 1 && String(Moghunter.hpgauge_NumberAfter) != "true") {
    this._IDA = true;
  }
...
HPGaugeSprite.prototype.createMeter = function () {
  ...
  for (var i = 0; i < 2; i++) {
    ...
    if (this.type === 0) {
      this._meter[i].x =
        Moghunter.hpgauge_a_xa +
        Moghunter.hpgauge_b_xa -
        this._meterImg.width / 2;
      this._meter[i].y =
        Moghunter.hpgauge_a_ya +
        Moghunter.hpgauge_b_ya -
        this._meterImg.height / 2;
    } else {
      this._meter[i].x =
        Moghunter.hpgauge_a_x +
        Moghunter.hpgauge_b_x -
        this._meterImg.width / 2;
      this._meter[i].y =
        Moghunter.hpgauge_a_y +
        Moghunter.hpgauge_b_y -
        this._meterImg.height / 2;
    }
...
HPGaugeSprite.prototype.needRefreshBlueMeter = function () {
  if (this.battler()._ehpgauge[4]) {
    return true;
  }
  if (this._meter[1].value1 != this.battler().hp) {
    return true;
  }
  if (this._meter[1].value2 != this.battler().mhp) {
    return true;
  }
  return false;
};
...
HPGaugeSprite.prototype.needRefreshRedMeter = function () {
  if (this._meter[0].value1 != this.battler().hp) {
    return true;
  }
  return false;
};
...
HPGaugeSprite.prototype.refreshRedMeter = function () {
  this._meter[0].value1 = this.update_dif(
    this._meter[0].value1,
    this.battler().hp,
    60,
    1
...
HPGaugeSprite.prototype.updateMeter = function () {
  if (this.needRefreshRedMeter()) {
    this.refreshRedMeter();
  }
  if (this.needRefreshBlueMeter()) {
    this.refreshBlueMeter();
  }
};
...
HPGaugeSprite.prototype.createNumber = function () {
  ...
  this._numberData = [
    this.battler().hp - 1,
    this._numberImg.width / 11,
    this._numberImg.height,
  ];
  for (var i = 0; i < 6; i++) {
...
HPGaugeSprite.prototype.updateNumber = function () {
  if (this._numberData[0] != this.battler().hp) {
    this.refreshNumber();
  }
};
...
HPGaugeSprite.prototype.refreshNumber = function () {
  this._numberData[0] = this.update_dif(
    this._numberData[0],
    this.battler().hp,
    40,
    0
...
HPGaugeSprite.prototype.forceVisible = function () {
  if (this.battler()._ehpgauge[3]) {
    return true;
  }
  if (
    this._selectionHP &&
    this.battler().isSelected() &&
    !this.battler().isDead()
  ) {
    return true;
  }
...
HPGaugeSprite.prototype.update = function () {
  ...
  if (!this._layout && this._meterImg.isReady()) {
    this.createSprites();
  }
  if (this._meter) {
    this.updateMeter();
