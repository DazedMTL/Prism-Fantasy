 ...
 */
//* // merusaiaコメント：なぜかここに「*」だけが一行だけあってSyntaxError…。他のプラグインにもたくさんあるけど…ミス？
/*:ja
 ...
 * @default 決定ボタン→変更したいボタンの順に押すと、変更できます
 *
 * @param Cancel Button
 * @desc キャンセルボタン
 ...
 * @default メニューの開閉や、キャンセルする時に使うボタンです
 *
 * @param Shift Button
 ...
 * @default ダッシュ時に押すボタンです（常時ダッシュがONなら歩行）
 *
 * @param Menu Button
 ...
 * @default メインメニューを開いたり閉じたりするボタンです
 *
 * @param PageUp Button
 ...
 * @default メニュー/ショップ時、前のキャラに切り替えます
 *
 * @param PageDown Button
 ...
 * @default メニュー/ショップ時、次のキャラに切り替えます
 *
 * @param Reset Default
 ...
 * @default 保存して完了します。次回起動時もこの設定が反映されます
 *
 * @help
 * ※ツクール公式のゲームパッドプラグインを同内容です。
 * 他のプラグインと競合しないような対策と、デフォルトのキャプションだけを、merusaiaが変更しています。
 * 必要に応じて、プラグイン編集画面でキャプションを変更してください。
 * ※ゲームパッドが検出されない場合、オプション項目自体が追加されないので、デバッグ時は注意してください。
 *
 * ゲームパッドが検出された場合、
 * 自動的に、"コントローラ設定"がオプション項目に追加されます。
 * プレイヤーは好みのボタンを変更することができ、
 * その設定は毎回プレイ時に自動的にロードされます。
 *
 * なお、ゲームパッドがオプションやゲームパッドコンフィグから検出されなくなると、
...
//=============================================================================
var _GamepadConfig_Window_Options_addGeneralOptions =
  Window_Options.prototype.addGeneralOptions;
Window_Options.prototype.addGeneralOptions = function () {
  _GamepadConfig_Window_Options_addGeneralOptions.apply(this, arguments); // 上書き後メソッドの最初に、上書き前メソッドを呼び出す。
  this.addGameConfigCommand(); // この行から、上書き後のメソッドの処理を実行。
};
...
Yanfly.GamepadConfig.Window_Options_update = Window_Options.prototype.update;
