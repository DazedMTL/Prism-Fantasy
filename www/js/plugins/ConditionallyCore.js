...
(function () {
              ...
              switch (ary[t]) {
                ...
                case "value2":
                  return [
                    i,
                    j,
                    [parseInt(RegExp.$1), parseInt(RegExp.$2), RegExp.$3],
                  ];
                case "value3":
  ...
  ConditionallyManager.checkConditions = function (
    user,
    target,
    conditions,
    dieStatus
  ...
  ConditionallyManager.checkTurn = function (targets, conditions) {
    var turn = this._dieStatus
      ? this._dieStatus["turn"]
      : BattleManager.ccTurnCount();
  ...
  ConditionallyManager.checkStatus = function (targets, conditions) {
    ...
    for (var i = 0; i < targets.length; i++) {
      ...
      if (this.checkValues(code, num, value1, value2)) {
        return true;
      }
    }
    return false;
    ...
    for (var i = 0; i < targets.length; i++) {
      ...
      for (var j = 0; j < conditions.length; j++) {
        ...
        switch (eVar) {
          case "W":
          case "w":
            if (targets[i].isEquipped($dataWeapons[eId])) {
              count++;
            }
            break;
          case "A":
          case "a":
            if (targets[i].isEquipped($dataArmors[eId])) {
              count++;
            }
            break;
        }
  ...
  ConditionallyManager.checkClass = function (targets, conditions) {
    ...
    var count = 0;
    for (var i = 0; i < targets.length; i++) {
      if (conditions.indexOf(targets[i]._classId) !== -1) {
        count++;
      }
      if (count >= num) {
        return true;
  ...
  ConditionallyManager.checkElement = function (targets, conditions) {
    ...
    var elements =
      item.damage.elementId < 0
        ? finisher.attackElements()
  ...
  ConditionallyManager.checkBattleMember = function (targets, conditions) {
    ...
    if (targets[0].isEnemy()) {
      members = $gameTroop.members().filter(function (enemy) {
