...
(function (global) {
  ...
  TextRuby.settings = (function () {
    ...
    return {
      ...
      ignoreEscapeCharacters:
        String(parameters["Ignore Escape Characters"] || "false") === "true",
  ...
  TextRuby.strictParseRubyParams = function (textState) {
    ...
    for (
      ;
      parseTextState.index < parseTextState.text.length;
          ...
          switch (text[parseTextState.index]) {
            ...
            case TextRuby.delimiter[phase].end:
              ...
              if (nest === 0) {
                phase = subTextExists
                  ? phase + 1
                  : TextRuby.strictParsePhaseEnd;
              }
              break;
            case "\x1b":
              Window_Base.prototype.obtainEscapeCode.call(null, parseTextState);
              Window_Base.prototype.obtainEscapeParam.call(
                null,
                parseTextState
              );
              parseTextState.index--;
  ...
  TextRuby.processDrawRuby = function (mainText, subText, textState) {
    ...
    var mainY =
      textState.y +
      (textState.height - originalFontSize) / 2 +
      (originalFontSize - mainFontSize);
  ...
  var upstream_Window_Base_processEscapeCharacter =
    Window_Base.prototype.processEscapeCharacter;
  Window_Base.prototype.processEscapeCharacter = function (code, textState) {
    upstream_Window_Base_processEscapeCharacter.apply(this, arguments);
    if (code !== "RUBY") return;
    var params = TextRuby.settings.ignoreEscapeCharacters
      ? TextRuby.strictParseRubyParams(textState)
      : TextRuby.obtainRubyParams(textState);
    if (!params) return;
    params[1] =
      params[1] ||
      TextRuby.commandDictionaries[params[0]] ||
      TextRuby.settings.dictionaries[params[0]];
