//
//  メニューコマンド並び替え ver1.02
//
// author yana
//

var Imported = Imported || {};
Imported["MenuCommandSort"] = 1.02;

/*:
 * @plugindesc ver1.02/メニューコマンドを並び替えたり表示非表示をオンオフできるようにします。
 * @author Yana
 *
...
(function () {
  ...
  var disableCommandText = String(
    parameters["Disable Command Text"] || "----------"
  );
  var loadName = String(parameters["Load Name"] || "ロード");
  var menuList = String(
    parameters["Menu List"] ||
  ...
  var _mCS_WMCommand_makeCommandList =
  ...
  Window_MenuCommand.prototype.sortCommand = function () {
    ...
    for (var i = 0; i < ary.length; i++) {
      ...
      if (commandSetting === 0 && !$gameSwitches.value(enableSwitchId + i)) {
        continue;
      }
  ...
  var _mCS_WMCommand_addOriginalCommands =
  ...
  Scene_Menu.prototype.commandCommonMenu = function () {
    ...
    if (c[3]) {
      $gameVariables.setValue(
        Number(c[3]),
        $gameParty.members()[this._statusWindow.index()]._actorId
      );
    }
