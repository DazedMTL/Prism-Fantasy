...
Moghunter.picturegallery_picture_number = Number(
  Moghunter.parameters["Number of Pictures"] || 3
);
Moghunter.picturegallery_setWallpaper = String(
  Moghunter.parameters["Set Wallpaper"] || "true"
);
Moghunter.picturegallery_command_menu = String(
  Moghunter.parameters["Command Menu"] || "true"
);
Moghunter.picturegallery_command_name = String(
  Moghunter.parameters["Command Word"] || "Picture Gallery"
);
Moghunter.picturegallery_completion_word = String(
  Moghunter.parameters["Completion Word"] || "Completion"
);
Moghunter.picturegallery_number_word = String(
  Moghunter.parameters["Number Word"] || "Pic"
);
Moghunter.picturegallery_thumbnail_x = Number(
  Moghunter.parameters["Thumbnail X-Axis"] || 23
);
Moghunter.picturegallery_thumbnail_y = Number(
  Moghunter.parameters["Thumbnail Y-Axis"] || 25
);
Moghunter.picturegallery_cols = Number(
  Moghunter.parameters["Thumbnails For Line"] || 3
);
Moghunter.picturegallery_number_x = Number(
  Moghunter.parameters["Number X-Axis"] || 0
);
Moghunter.picturegallery_number_y = Number(
  Moghunter.parameters["Number Y-Axis"] || -32
);
Moghunter.picturegallery_double_click_speed = Number(
  Moghunter.parameters["Double Click Speed"] || 10
);
Moghunter.picturegallery_directory = String(
  Moghunter.parameters["File Directory"] || "img/pictures/"
);
Moghunter.picturegallery_file_name = String(
  Moghunter.parameters["File Name"] || "Pic_"
);
Moghunter.picturegallery_fit_screen_key = String(
  Moghunter.parameters["Fit Screen Key"] || "pagedown"
);
Moghunter.picturegallery_wallpaper_key = String(
  Moghunter.parameters["Set Wallpaper Key"] || "pageup"
);
Moghunter.picturegallery_info = String(
  Moghunter.parameters["Info Visible"] || "true"
);
Moghunter.picturegallery_infoX = Number(
  Moghunter.parameters["Inof X-Axis"] || 0
);
Moghunter.picturegallery_infoY = Number(
  Moghunter.parameters["Inof Y-Axis"] || 0
);
Moghunter.picturegallery_infoDuration = Number(
  Moghunter.parameters["Info Duration"] || 90
);
...
Game_System.prototype.make_picture_list = function () {
  ...
  for (var i = 0; i < Moghunter.picturegallery_picture_number; i++) {
    this._picgl_data[i] = [
      false,
      String(Moghunter.picturegallery_file_name + (i + 1)),
    ];
  }
};
...
Game_System.prototype.enable_picture = function (value, enable) {
  ...
  if (!this._picgl_data || !this._picgl_data[pic_id]) {
    return;
  }
  if (!this._picgl_data[pic_id]) {
    this._picgl_data[pic_id] = [true, ""];
  }
  ...
  if (!this._picgl_data || this._picgl_data.length === 0) {
    ...
    alert(
      "There are no " +
        ...
        ".png files in this /" +
        Moghunter.picturegallery_directory +
        " folder..."
    );
    SceneManager.exit();
    return;
...
// * Initialize
//==============================
Window_PictureList.prototype.initialize = function (
  x,
  y,
  width,
  height,
...
Window_PictureList.prototype.drawItem = function (i) {
  if (this._data[i]) {
    ...
    if (!this._pic_thumb[i]) {
      this.create_thumb(i, rect);
    }
...
Window_PictureList.prototype.refresh_position = function (i, rect) {
  ...
  this._pic_name[i].x =
    this._pic_thumb[i].x + Moghunter.picturegallery_number_x;
  this._pic_name[i].y =
    this._pic_thumb[i].y + rect.height + Moghunter.picturegallery_number_y;
...
Window_PictureList.prototype.create_thumb = function (i, rect) {
  ...
  this._pic_name[i].bitmap.drawText(
    Moghunter.picturegallery_number_word + " " + String(i + 1),
    0,
    0,
    rect.width - 10,
    32,
...
if (String(Moghunter.picturegallery_command_menu) === "true") {
  ...
  // * make Command List
  //==============================
  var _alias_mog_picgal_addOriginalCommands =
  ...
  Window_MenuCommand.prototype.addPictureGallery = function () {
    this.addCommand(
      String(Moghunter.picturegallery_command_name),
      "picture_gallery",
      true
    );
  };
  ...
  //==============================
  var _alias_mog_picgal_reateCommandWindow =
    Scene_Menu.prototype.createCommandWindow;
  Scene_Menu.prototype.createCommandWindow = function () {
    _alias_mog_picgal_reateCommandWindow.call(this);
    this._commandWindow.setHandler(
      "picture_gallery",
      this.commandPictureGallery.bind(this)
    );
...
Scene_Picture_Gallery.prototype.initialize = function () {
  ...
  this._wallpaper =
    String(Moghunter.picturegallery_setWallpaper) === "true" ? true : false;
...
Scene_Picture_Gallery.prototype.createInfo = function () {
  ...
  this._info._index = 0;
  this._info.org = [
    Moghunter.picturegallery_infoX,
    Moghunter.picturegallery_infoY,
  ];
...
Scene_Picture_Gallery.prototype.create_window_list = function () {
  ...
  this._w_list = new Window_PictureList(
    x,
    y,
    w,
    h,
...
Scene_Picture_Gallery.prototype.set_zoom = function (value) {
  ...
  if (this._picture.scale.x > 1.5) {
    this._picture.scale.x = 1.5;
  }
  if (this._picture.scale.x < 1.0) {
    this._picture.scale.x = 1.0;
  }
...
Scene_Picture_Gallery.prototype.update_Input = function () {
  ...
  if (Input.isPressed("right")) {
    this.move_to(0, -this.move_speed());
  }
  if (Input.isPressed("left")) {
    this.move_to(0, this.move_speed());
  }
  if (Input.isPressed("down")) {
    this.move_to(1, -this.move_speed());
  }
  if (Input.isPressed("up")) {
    this.move_to(1, this.move_speed());
  }
  if (Input.isTriggered(this._fit_screen_key)) {
    this.changePictureMode();
  }
  if (Input.isTriggered(this._wallpaper_key)) {
    this.setBackground();
  }
};
...
Scene_Picture_Gallery.prototype.setBackground = function () {
  $gameSystem._backgroundName = String(
    Moghunter.picturegallery_file_name + (this._w_list._index + 1)
  );
  this.set_FullMode();
  this._info._phase[0] = 0;
...
Scene_Picture_Gallery.prototype._moveTo_TouchInput = function () {
  ...
  if (!this._txv) {
    this._txv = [
      this._picture.x + TouchInput._x,
      this._picture.y + TouchInput._y,
    ];
