...
(function () {
  ...
  Window_Monitor.prototype.update = function () {
    ...
    if (Input.isTriggered("tab") && this.visible)
    ...
    for (var i = 0; i < this._list.length; i++) {
      ...
      if (k === "S") {
        ...
        var newValue =
          (name || "S[" + n.padZero(4) + "]") +
          ":" +
          ($gameSwitches.value(n) ? "[ON]" : "[OFF]");
      } else {
        var name = $dataSystem.variables[n];
        var newValue =
          (name || "V[" + n.padZero(4) + "]") + ":" + $gameVariables.value(n);
      }
  ...
  Window_DebugEdit.prototype.itemStatus = function (dataId) {
    if (this._mode === "switch") {
      return (
        ($gameSwitches.value(dataId) ? "[ON]" : "[OFF]") +
        (switches[dataId] ? "*" : "")
      );
    } else {
      return (
        String($gameVariables.value(dataId)) + (variables[dataId] ? "*" : "")
      );
    }
  };

  //リアルタイム変更モードの時、他のメニューのカーソルが動いたりプレイヤーが歩いたりしないようにする
  var _Window_Selectable_isOpenAndActive =
  ...
  Scene_Map.prototype.createMonitorWindow =
    Scene_Battle.prototype.createMonitorWindow = createMonitorWindow;
})();
