...
/*:
 * @target MZ
 * @plugindesc v1.361 Creates a combination skill.
...
(function () {
  ...
  var pShowPartyCommandPosition = toNumber(
    parameters["showPartyCommandPosition"],
    1
  );
  var pShowMenuCommand = toBoolean(parameters["showMenuCommand"], false);
  var pShowMenuCommandPosition = toNumber(
  ...
  Game_BattlerBase.prototype.canUse = function (item) {
    ...
    if (isCombinationSkill(item)) {
      if (DataManager.isSkill(item) && canUseCombinationSkill(this, item)) {
        ...
        canUseCombinationItem(this, item)
      ) {
        return true;
      }
      return false;
   ...
   * ※標準ではモーションの調整のみ
   */
  const _Window_BattleLog_performActionStart =
   ...
   * ※標準ではモーションの調整のみ
   */
  var _Window_BattleLog_performActionEnd =
  ...
  function getCombinationMembers(item, subject) {
    ...
    if (csActorsMeta) {
      // 配列化
      var actorsId = csActorsMeta.split(",");
      for (let i = 0; i < actorsId.length; i++) {
        var actorId = Number(actorsId[i]);
      ...
      for (let i = 0; i < battlersIndex.length; i++) {
        // ０始まりに変換
        var index = Number(battlersIndex[i]) - 1;
   ...
   * ●前のコマンドへ
   */
  var _Game_Actor_selectPreviousCommand =
    Game_Actor.prototype.selectPreviousCommand;
  ...
  Window_PartyCommand.prototype.makeCommandList = function () {
    ...
    // 非表示スイッチがオンの場合
    if (
      pHidePartyCommandSwitch &&
      $gameSwitches.value(pHidePartyCommandSwitch)
    ) {
      return;
    }
    ...
    if (pShowPartyCommand) {
      this._list.splice(pShowPartyCommandPosition, 0, {
        name: pCombinationName,
        symbol: pCombinationSymbol,
        enabled: true,
        ext: null,
      });
    }
  ...
  Scene_Battle.prototype.createPartyCommandWindow = function () {
    ...
    this._partyCommandWindow.setHandler(
      pCombinationSymbol,
      this.commandCombination.bind(this)
    );
  };
  ...
  Scene_Menu.prototype.createCommandWindow = function () {
    ...
    this._commandWindow.setHandler(
      pCombinationSymbol,
      this.commandCombinationSkill.bind(this)
    );
  };
   ...
   */
  const _Game_Battler_removeCurrentAction =
