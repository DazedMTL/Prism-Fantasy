...
(function () {
  ...
  var unselectableTargetColor = Number(
    parameters["Unselectable Target Color"] || 5
  );
  ...
  var _tOSU_GBBase_meetsSkillConditions =
  ...
  var _tOSU_GBBase_meetsItemConditions =
  ...
  Game_BattlerBase.prototype.isUsableCond = function (item) {
    ...
    for (var i = 0; i < item._useCond.length; i++) {
      ...
      if ([1, 2, 3, 4, 5, 6].contains(item.scope)) {
        ...
        if (!this.checkUseCond([user, target, cond])) {
          return false;
        }
      }
    }
  ...
  Game_BattlerBase.prototype.lastCond = function (item) {
    ...
    if (this._lastCond[type][item.id] === undefined) {
      return true;
    }
  ...
  var _tOSU_GAction_itemTargetCandidates =
  ...
  var _tOSU_SBattle_selectActorSelection =
  ...
  var _tOSU_SBattle_selectEnemySelection =
