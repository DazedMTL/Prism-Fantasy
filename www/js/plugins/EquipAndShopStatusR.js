...
(function () {
  ...
  var damageTypeText = String(
    parameters["Damage Type Text"] || "ダメージタイプ"
  );
  ...
  var scopeVocab = String(
    parameters["Scope Vocab"] ||
      "なし,敵単体,敵全体,敵1体 ランダム,敵2体 ランダム,敵3体 ランダム,敵4体 ランダム,味方単体,味方全体,味方単体(戦闘不能),味方全体(戦闘不能),使用者"
  ).split(",");
  var occasionVocab = String(
    parameters["Occasion Vocab"] || "常時,バトル,メニュー,使用不可"
  ).split(",");
  var damageVocab = String(
    parameters["Damage Vocab"] ||
      "なし,HPダメージ,MPダメージ,HP回復,MP回復,HP吸収,MP吸収"
  ).split(",");
  var itemTypeVocab = String(
    parameters["Item Type Vocab"] ||
      "通常アイテム,貴重品,隠しアイテムA,隠しアイテムB"
  ).split(",");
  var consumeVocab = String(
    parameters["Consume Vocab"] || "消耗する,消耗しない"
  ).split(",");
  var xparamNames = String(
    parameters["Xparam Names"] ||
      "命中,回避,クリティカル率,魔法回避,会心回避,魔法反射,反撃,HP再生率,MP再生率,TP再生率"
  ).split(",");
  var sparamNames = String(
    parameters["Sparam Names"] ||
      "狙われ率,防御効果率,回復効果率,薬の知識,MP消費率,TPチャージ率,物理ダメージ率,魔法ダメージ率,床ダメージ率,経験値獲得率"
  ).split(",");
  var effectNames = String(
    parameters["Effects Names"] ||
  ...
  var maxShopEquipPageLine = Number(
    parameters["Max Shop Equip Page Line"] || 7
  );
  ...
  var isRefineShopStatus =
    String(parameters["Is Refine Shop Status"]) === "true";
  var pageIconColor = String(
    parameters["Page Icon Color"] || "rgba(192,255,192,1.0)"
  );
  ...
  var performanceIconPositionY =
    Number(parameters["Performance Icon Position Y"]) || 18;
  ...
  Game_Actor.prototype.pictureName = function () {
    ...
    if (this._pictureName === undefined) {
      this._pictureName = "";
      if (this.actor().meta["StatusPicture"]) {
        this._pictureName = this.actor().meta["StatusPicture"];
      }
      if (this.actor().meta["ステータスピクチャ"]) {
        this._pictureName = this.actor().meta["ステータスピクチャ"];
      }
    }
  ...
  Window_Base.prototype.drawStandup = function (
    name,
    x,
    y,
    opacity,
  ...
  Window_EquipStatus.prototype.refreshContents = function () {
    ...
    if (this.maxPage() > 1)
      this.drawPageIcon(lineHeight * max + pageIconOffsetY);

    for (var i = 0; i < max; i++) {
      var bs = basicParams[i];
      switch (bs) {
        case "name":
          this.drawActorName(this._actor, this.textPadding(), yy);
          break;
        case "level":
          this.drawActorLevel(this._actor, this.textPadding(), yy);
          break;
        case "class":
          this.drawActorClass(this._actor, this.textPadding(), yy);
          break;
        case "states":
          this.drawActorIcons(this._actor, this.textPadding(), yy);
          break;
        case "nickname":
          this.drawActorNickname(this._actor, this.textPadding(), yy);
          break;
        case "s":
          break;
        case "l":
          this.drawHorzLine(yy + this.fontSize() / 2 + 2);
          break;
        default:
          if (bs.match(/meta\[(.+)\]/i)) {
            this.drawTextEx(
              this._actor.actor().meta[RegExp.$1],
              this.textPadding(),
              yy
            );
            break;
          } else {
            this.drawTextEx(bs, this.textPadding(), yy);
            break;
          }
      }
      yy += lineHeight;
    ...
    if (this._tempActor && equipStatusHelpText2) {
      ...
      this.drawText(
        equipStatusHelpText2,
        16,
        this.contentsHeight() - 28,
        this.contentsWidth() - 24
      );
  ...
  Window_EquipStatus.prototype.drawExParams = function (
    text,
    value,
    color,
    x,
  ...
  Window_EquipStatus.prototype.countDrawStatus = function () {
    ...
    if (
      ...
      !this._actor.equips().contains(item)
    ) {
      ...
      for (var i = 0, max = texts.length; i < max; i++) {
        ...
        if (
          text.match(
            /<(?:装備制限表示|DisplayEquippedLimit|追加表示|DisplayExtend)[:：](.+)?,(.+)>/
          )
        )
          l++;
      }
    ...
    for (var i = 0; i < lSt; i++) {
      ...
      var sta = states[i].id;
      if (this._actor.stateRate(sta) !== this._tempActor.stateRate(sta)) {
        l++;
      }
    }
    return l;
  ...
  Window_EquipStatus.prototype.drawPageIcon = function (y) {
    for (var i = 0; i < this.maxPage(); i++) {
      ...
      this.contents.fillRect(
        this.contentsWidth() - 12 - 14 * this.maxPage() + i * 14,
        y,
        12,
        12,
        color
  ...
  Window_EquipStatus.prototype.drawChangeStatus = function (x, y, size) {
    ...
    for (var i = 0; i < 8; i++) {
      if (
        this._actor.param(i) !== this._tempActor.param(i) &&
        this.includeEnableParams(i)
      ) {
        if (l >= 0) {
          this.drawLineStatus(i, x, y + size * l);
        }
        l++;
      }
      if (l >= this.maxPageLine()) {
        return;
      }
    }
    for (var i = 0; i < 10; i++) {
      if (
        this._actor.xparam(i) !== this._tempActor.xparam(i) &&
        this.includeEnableParams(i + 8)
      ) {
        if (l >= 0) {
          this.drawLineStatus(i + 8, x, y + size * l);
        }
        l++;
      }
      if (l >= this.maxPageLine()) {
    ...
    for (var i = 0; i < 10; i++) {
      if (
        this._actor.sparam(i) !== this._tempActor.sparam(i) &&
        this.includeEnableParams(i + 18)
      ) {
        if (l >= 0) {
          this.drawLineStatus(i + 18, x, y + size * l);
        }
        l++;
      }
      if (l >= this.maxPageLine()) {
    ...
    for (var i = 0; i < lEle; i++) {
      if (
        this._actor.elementRate(i) !== this._tempActor.elementRate(i) &&
        this.includeEnableParams(i + 28)
      ) {
        if (l >= 0) {
          this.drawLineStatus(i + 28, x, y + size * l);
        }
        l++;
      }
      if (l >= this.maxPageLine()) {
    ...
    for (var i = 0; i < lSt; i++) {
      ...
      var sta = states[i].id;
      if (
        this._actor.stateRate(sta) !== this._tempActor.stateRate(sta) &&
        this.includeEnableParams(sta + 28 + lEle)
      ) {
        if (l >= 0) {
          this.drawLineStatus(sta + 28 + lEle, x, y + size * l);
        }
        l++;
      }
      if (l >= this.maxPageLine()) {
    ...
    for (var i = 0; i < 8; i++) {
      if (
        this._actor.debuffRate(i) !== this._tempActor.debuffRate(i) &&
        this.includeEnableParams(i + 28 + lEle + lSt)
      ) {
        if (l >= 0) {
          this.drawLineStatus(i + 28 + lEle + lSt, x, y + size * l);
        }
        l++;
      }
    }
  ...
  Window_EquipStatus.prototype.includeEnableParams = function (paramId) {
    ...
    if (paramId < 28) {
      ...
      paramId <
      28 + $dataSystem.elements.length + $dataStates.length
    ) {
  ...
  Window_EquipStatus.prototype.drawLineStatus = function (
    index,
    x,
    y,
    eraseArrow
    ...
    if (this._actor) {
      this.drawParams(0, index, x + x2, y);
    }
    if (!eraseArrow) {
      this.drawRightArrow(x + x3, y);
    }
    if (this._tempActor) {
      this.drawParams(1, index, x + x4, y);
    }
  };
  ...
  Window_EquipStatus.prototype.drawWLParameter = function (trg, paramId, x, y) {
    var ww = 132;
    var noWeaponTypeName =
      PluginManager.parameters("StatusUpReward")["NoWeaponTypeName"];
    var levelNameCompact =
      PluginManager.parameters("StatusUpReward")["LevelNameCompact"];
    this.changeTextColor(this.normalColor());
    var slotIndex = SceneManager._scene._slotWindow.index();
    ...
    if (id === 0 && actor.weapons().length > 0) {
      ...
      this.drawText(
        levelNameCompact + actor.weaponLevel(id),
        x,
        y,
        ww,
        "right"
  ...
  Window_EquipStatus.prototype.drawParams = function (trg, paramId, x, y) {
    ...
    if (paramId < 8) {
      ...
      this.drawText(
        Math.floor(actor.xparam(paramId - 8) * 100) + "%",
        x,
        y,
        ww,
        "right"
      ...
      this.drawText(
        Math.floor(actor.sparam(paramId - 18) * 100) + "%",
        x,
        y,
        ww,
        "right"
      ...
      this.drawText(
        Math.floor(actor.elementRate(paramId - 28) * 100) + "%",
        x,
        y,
        ww,
        "right"
      ...
      this.drawText(
        Math.floor(actor.stateRate(paramId - (28 + lEle)) * 100) + "%",
        x,
        y,
        ww,
        "right"
      ...
      this.drawText(
        Math.floor(actor.debuffRate(paramId - (28 + lEle + lSta)) * 100) + "%",
        x,
        y,
        ww,
        "right"
  ...
  Window_EquipStatus.prototype.changeColorParam = function (paramId) {
    ...
    if ([22, 24, 25, 26].contains(paramId)) {
      change *= -1;
    }
  ...
  Window_EquipStatus.prototype.maxPage = function () {
    if (this._tempActor) {
      ...
      if (!this._maxPage) {
        this._maxPage = [page1, page2, page3, page4, page5, page6].reduce(
          function (c, a) {
            if (a[0]) {
              c += 1;
            }
  ...
  Window_EquipStatus.prototype.prevPage = function () {
    if (this.maxPage() > 1) {
      SoundManager.playCursor();
      this._pageIndex =
        (this._pageIndex + (this.maxPage() - 1)) % this.maxPage();
      this.refresh();
    }
  };
  ...
  Window_EquipSlot.prototype.processRelease = function () {
    if (Imported["LimitPossession"]) {
      if (
        $gameParty.allItemsWeight() + DataManager.itemWeight(this.item()) >
        $gameParty.maxWeight()
      ) {
  ...
  Scene_Equip.prototype.smallSlotWindow = function () {
    ...
    this._slotWindow.height =
      this._slotWindow.contents.fontSize +
  ...
  Window_RefineShopStatus.prototype = Object.create(
  ...
  Window_RefineShopStatus.prototype.initialize = function (
    x,
    y,
    width,
    height
  ...
  Window_RefineShopStatus.prototype.drawEquipItemStatus = function (x, y) {
    ...
    for (var i = 0; i < equipInfo.length; i++) {
      ...
      switch (equipInfo[i]) {
        case "price":
          var price = this._item.price;
          if (this._item.meta["value"]) {
            price = this._item.meta["value"];
          }
          if (this._item.meta["価値"]) {
            price = this._item.meta["価値"];
          }
          if (price > 0) {
        ...
        case "l":
          if (l > 0) {
            this.drawHorzLine(size * l + yy + 8 + size / 2);
          }
          break;
        default:
    ...
    for (var i = 0; i < shopInfo.length; i++) {
      ...
      switch (shopInfo[i]) {
        ...
        case "price":
          var price = this._item.price;
          if (this._item.meta["value"])
            price = Number(this._item.meta["value"]);
        ...
        case "repeat":
          if (this._item.repeats > 1) {
            if (l >= 0)
              this.drawRepeat(this._item.repeats, xxx, yy + size * l + 4);
          } else {
            l--;
          }
          break;
        case "tpGain":
          if (this._item.tpGain !== 0) {
            if (l >= 0)
              this.drawTpGain2(this._item.tpGain, xxx, yy + size * l + 4);
          } else {
            l--;
          }
          break;
        ...
        case "weight":
          if (l >= 0 && Imported["LimitPossession"]) {
            ...
            if (weight > 0) {
              var s =
                PluginManager.parameters("LimitPossession")[
    ...
    for (var i = 0; i < mLength; i++) {
      ...
      if (member.isEquipped(this._item)) {
        ...
        if (a < b) {
          ...
          if (Imported["EquippedLimit"] && !member.isLeEquippable(this._item))
            this.changeTextColor(this.textColor(4));
          this.contents.fontSize = 16;
          this.drawText(
            performanceIconUp,
            xxx + 4,
            yyy + performanceIconPositionY
          );
        } else if (a > b) {
          this.changeTextColor(this.paramchangeTextColor(b - a));
          if (Imported["EquippedLimit"] && !member.isLeEquippable(this._item))
  ...
  Window_RefineShopStatus.prototype.checkCodeEnabled = function (code) {
    switch (code) {
      case 11:
        return !!effectNames[0] && !!effectNames[1];
      case 12:
        return !!effectNames[2] && !!effectNames[3];
      case 13:
        return !!effectNames[4];
      case 21:
        return !!effectNames[5];
      case 22:
        return !!effectNames[6];
      case 31:
        return !!effectNames[7];
      case 32:
        return !!effectNames[8];
      case 33:
        return !!effectNames[9];
      case 34:
        return !!effectNames[10];
      case 41:
        return !!effectNames[11];
      case 42:
        return !!effectNames[12];
      case 43:
        return !!effectNames[13];
      case 44:
        return !!effectNames[14];
    }
    return true;
  };
  ...
  Window_RefineShopStatus.prototype.drawEffectText = function (
    text1,
    text2,
    x,
    y
  ...
  Window_RefineShopStatus.prototype.drawRecover = function (
    type,
    value1,
    value2,
    x,
  ...
  Window_RefineShopStatus.prototype.drawState = function (
    type,
    dataId,
    value1,
    x,
  ...
  Window_RefineShopStatus.prototype.drawBuff = function (
    type,
    dataId,
    value1,
    x,
  ...
  Window_RefineShopStatus.prototype.drawRemoveBuff = function (
    type,
    dataId,
    x,
    y
  ...
  Window_RefineShopStatus.prototype.drawGrowEffect = function (
    dataId,
    value1,
    x,
    y
