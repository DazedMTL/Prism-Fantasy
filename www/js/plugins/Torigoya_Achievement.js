...
(function (global) {
  ...
  Achievement.settings = (function () {
    ...
    return {
      ...
      popupOpacity: Number(
        parameters["Popup Opacity"] === undefined
          ? -1
          : parameters["Popup Opacity"]
      ),
      listHiddenTitle: String(parameters["List Hidden Title"] || "？？？？？"),
      listHiddenDescription: String(
        parameters["List Hidden Description"] || ""
      ),
  ...
  Achievement.loadFromCommonEvent = function (commonEventID) {
    ...
    for (var i = 0; i < list.length; ++i) {
      if (isRouting && list[i].code === 408) {
        ...
        list[i].code === 108 &&
        list[i].parameters[0].indexOf("id:") === 0
      ) {
  ...
  var AchievementManager = (function () {
    ...
    AchievementManager.prototype.init = function () {
      this._items = Achievement.loadFromCommonEvent(
        Achievement.settings.commonEventID
      );
      this.load();
    };
  ...
  if (!Achievement.settings.useGlobalSave) {
    ...
    var upstream_DataManager_extractSaveContents =
      DataManager.extractSaveContents;
    DataManager.extractSaveContents = function (contents) {
      upstream_DataManager_extractSaveContents.apply(this, arguments);
      Achievement.Manager.setRawAchievements(
        contents.torigoyaAchievements || []
      );
    };
  }
    ...
    AchievementPopupManager.prototype.onNotify = function (item) {
      ...
      Torigoya.Tween.create(window, {
        ...
        .to(
          ...
          30,
          Torigoya.Tween.Easing.easeOutCircular
        )
        ...
        .to(
          ...
          30,
          Torigoya.Tween.Easing.easeInCircular
        )
        .onComplete(
          function () {
  ...
  Window_AchievementPopup.prototype.drawMessage = function () {
    ...
    this.contents.drawText(
      Achievement.settings.popupMessage,
      50,
      29,
      textWidth,
      12,
  ...
  Window_AchievementPopup.prototype.loadWindowskin = function () {
    this.windowskin = ImageManager.loadSystem(
      Achievement.settings.popupWindowImage
    );
  };
  ...
  Window_AchievementPopup.prototype.standardBackOpacity = function () {
    return Achievement.settings.popupOpacity === -1
      ...
      : Achievement.settings.popupOpacity;
  };
  ...
  Window_AchievementList.prototype.makeItemList = function () {
    this._data = AchievementManager.allData().map(function (achievement) {
      ...
      if (!unlocked && achievement.isSecret) {
        return {
          title:
            achievement.secretTitle || Achievement.settings.listHiddenTitle,
          description:
            achievement.secretDescription ||
            Achievement.settings.listHiddenDescription,
  ...
  Window_AchievementList.prototype.drawItem = function (index) {
    ...
    if (item) {
      ...
      this.changePaintOpacity(true);
      this.drawText(
        Achievement.settings.listCancel,
        rect.x,
        rect.y,
        rect.width,
        "center"
  ...
  Scene_Achievement.prototype.createAchievementWindow = function () {
    this._achievementWindow = new Window_AchievementList(
      0,
      this._helpWindow.y + this._helpWindow.height,
      Graphics.boxWidth,
      Graphics.boxHeight - this._helpWindow.y - this._helpWindow.height
    );
  ...
  if (Achievement.settings.useTitle && Achievement.settings.useGlobalSave) {
    var upstream_Window_TitleCommand_makeCommandList =
    ...
    var upstream_Scene_Title_createCommandWindow =
      Scene_Title.prototype.createCommandWindow;
    Scene_Title.prototype.createCommandWindow = function () {
      upstream_Scene_Title_createCommandWindow.apply(this);
      this._commandWindow.setHandler(
        "achievement",
        this.torigoyaAchievement_commandAchievement.bind(this)
      );
    };
  }
  ...
  if (Achievement.settings.useMenu) {
    var upstream_Window_MenuCommand_addOriginalCommands =
    ...
    var upstream_Scene_Menu_createCommandWindow =
  ...
  var upstream_Game_Interpreter_pluginCommand =
