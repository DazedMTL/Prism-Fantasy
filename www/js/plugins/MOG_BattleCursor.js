...
Moghunter.bcursor_float = String(
  Moghunter.parameters["Float Effect"] || "true"
);
Moghunter.bcursor_name_visible = String(
...
Moghunter.bcursor_sort_x = String(
  Moghunter.parameters["Sort X-Axis"] || "true"
);
Moghunter.bcursor_window = String(
  Moghunter.parameters["Window Visible"] || "false"
);
Moghunter.bcursor_touch_selection = String(
  Moghunter.parameters["Touch Selection"] || "true"
);
Moghunter.bcursor_helpAllAllies = String(
  Moghunter.parameters["Help All Allies"] || "All Allies"
);
Moghunter.bcursor_helpAllEnemies = String(
  Moghunter.parameters["Help All Enemies"] || "All Enemies"
);
...
// * onSelectAction
//==============================
var _alias_mog_battle_cursor_onSelectAction =
...
if (Imported.MOG_BattleHud) {
  ...
  Battle_Hud.prototype.update_face = function () {
    ...
    if (
      this._battler._bhfaceSize[0] === -1 &&
      this._face &&
      this._face.bitmap.isReady()
    ) {
...
// * create Spriteset
//==============================
var _mog_bcursor_sbattle_createSpriteset =
...
//==============================
var _mog_bcursor_sbat_createDisplayObjects =
...
Scene_Battle.prototype.updateBattleCursorSB = function () {
  if ($gameTemp._needRefreshBattleCursor) {
    this.refreshBattlerCursor();
  }
};
...
BattleCursor.prototype.setup = function (spriteset, actorsprites) {
  ...
  if (String(Moghunter.bcursor_touch_selection) === "true") {
    this._touch_selection = true;
  }
  if (String(Moghunter.bcursor_name_visible) === "true") {
    this._arrow_name_visible = true;
  }
  if (String(Moghunter.bcursor_float) === "true") {
    this._arrow_float_effect = true;
  }
};
...
BattleCursor.prototype.createArrowActor = function () {
  this._actor_arrowVisible =
    !$gameSystem.isSideView() && !Imported.MOG_BattleHud ? false : true;
  for (var i = 0; i < $gameParty.maxBattleMembers(); i++) {
    this._actor_arrow[i] = new Sprite(
      ImageManager.loadSystem("BattleCursor_A")
    );
...
BattleCursor.prototype.createArrowEnemy = function () {
  for (var i = 0; i < this.enemies().length; i++) {
    this._enemy_arrow[i] = new Sprite(
      ImageManager.loadSystem("BattleCursor_B")
    );
...
BattleCursor.prototype.update = function () {
  ...
  if (Imported.MOG_ConsecutiveBattles && $gameSystem._consBatime > 0) {
    return;
  }
  this.x = this.field().x;
  this.y = this.field().y;
  for (var i = 0; i < this.enemies().length; i++) {
    ...
    if (this._arrow_name_visible) {
      this.update_arrow_name(this._enemy_name[i], sprite, battler, 0);
    }
  }
  if (this._actor_arrowVisible) {
    for (var i = 0; i < $gameParty.maxBattleMembers(); i++) {
      ...
      if (this._arrow_name_visible) {
        this.update_arrow_name(this._actor_name[i], sprite, battler, 1);
      }
    }
  }
  if (this._arrow_float_effect) {
    this.update_arrow_slide();
  }
  if (this._touch_selection) {
...
BattleCursor.prototype.isVisible = function () {
  if (Imported.MOG_ATB) {
    if ($gameSystem._atbEventPhase[3]) {
      return false;
    }
  }
  return true;
...
BattleCursor.prototype.updateTouchSelection = function () {
  if (TouchInput.isTriggered()) {
    for (var i = 0; i < this.enemies().length; i++) {
      if (
        this.isTouchOnTarget(this.enemies()[i], this.enemies()[i]._battler, 0)
      ) {
    ...
    for (var i = 0; i < this.actors().length; i++) {
      if (
        this.isTouchOnTarget(this.actors()[i], this.actors()[i]._battler, 1)
      ) {
        $gameTemp._arrowTarget[1] = this.actors()[i]._battler;
      }
      if (
        Imported.MOG_BattleHud &&
        this.isTouchOnTargetFace(this.actors()[i], this.actors()[i]._battler, 1)
      ) {
        $gameTemp._arrowTarget[1] = this.actors()[i]._battler;
      }
    }
...
BattleCursor.prototype.isTouchOnTargetFace = function (sprite, battler, type) {
  ...
  if (type === 0 && !battler.isAlive()) {
    return false;
  }
  if (type === 0 && battler.isDead()) {
    return false;
  }
  if (battler._bhfaceSize[0] < 0) {
    return;
  }
  var cw = battler._bhfaceSize[0] / 2;
  var ch = battler._bhfaceSize[1] / 2;
  if (sprite) {
    if (TouchInput.x < this.field().x + sprite.x - cw) {
      return false;
    }
    if (TouchInput.x > this.field().x + sprite.x + cw) {
      return false;
    }
    if (TouchInput.y > this.field().y + sprite.y + ch) {
      return false;
    }
    if (TouchInput.y < this.field().y + sprite.y - ch) {
      return false;
...
BattleCursor.prototype.isTouchOnTarget = function (sprite, battler, type) {
  ...
  if (sprite.bitmap) {
    ...
    if (battler.isEnemy() && Imported.MOG_EnemyPoses) {
      if (battler._batPoses[0]) {
        sw = Math.floor(sw / 4);
      }
    }
    if (TouchInput.x < this.field().x + sprite.x - sw / 2) {
      return false;
    }
    if (TouchInput.x > this.field().x + sprite.x + sw / 2) {
      return false;
    }
    if (TouchInput.y > this.field().y + sprite.y) {
      return false;
    }
    if (TouchInput.y < this.field().y + sprite.y - sprite.bitmap.height) {
      return false;
    }
    return true;
    ...
    if (
      TouchInput.x <
      this.field().x + sprite.x - sprite._mainSprite.width / 2
    ) {
      return false;
    }
    if (
    ...
    if (TouchInput.y < this.field().y + sprite.y - sprite._mainSprite.height) {
      return false;
    }
    return true;
  }
...
// * Update Arrow Name
//==============================
BattleCursor.prototype.update_arrow_name = function (
  sprite,
  target,
  battler,
  type
  ...
  if (sprite.opacity === 100 || $gameTemp._arrow_need_refresh) {
    this.refresh_arrow_name(battler, sprite);
  }
  if (battler._refCursor) {
    this.refresh_arrow_name(battler, sprite);
  }
...
BattleCursor.prototype.update_arrow = function (sprite, target, battler, type) {
  if (!this.isArrowVisible(sprite, target, battler, type)) {
    this.hide_arrow(sprite, type);
    return;
  }
  sprite.opacity = 255;
  sprite.visible = true;
  if (type === 0) {
    var yf = target.height / 2;
  } else {
    if (target._mainSprite) {
      var yf = target._mainSprite.height;
    } else {
      var yf = 0;
    }
  }
...
// * Update Arrow
//==============================
BattleCursor.prototype.updateArrowCamera = function (
  sprite,
  target,
  battler,
  type
...
// * Update Arrow Actor
//==============================
BattleCursor.prototype.updateArrowCameraActor = function (
  sprite,
  target,
  battler,
  type
  ...
  if (Imported.MOG_BattleHud && !$gameSystem.isSideView()) {
    ...
    var perc = ($gameTemp._bcam.scale / 1.0) * 100 - 100;
...
// * Update Arrow Enemy
//==============================
BattleCursor.prototype.updateArrowCameraEnemy = function (
  sprite,
  target,
  battler,
  type
...
// * Update Arrow Normal
//==============================
BattleCursor.prototype.updateArrowNormal = function (
  sprite,
  target,
  battler,
  type
...
// * Update Cursor Actor
//==============================
BattleCursor.prototype.isArrowVisible = function (
  sprite,
  target,
  battler,
  type
...
BattleCursor.prototype.update_arrow_slide = function () {
  this._arrow_s[4] += 1;
  if (this._arrow_s[4] < 2) {
    return;
  }
  this._arrow_s[4] = 0;
  this._arrow_s[3] += 1;
  if (this._arrow_s[3] < 10) {
    this._arrow_s[2] += 1;
  } else if (this._arrow_s[3] < 20) {
    this._arrow_s[2] -= 1;
  } else {
    this._arrow_s[2] = 0;
    this._arrow_s[3] = 0;
...
Sprite_Enemy.prototype.setBattler = function (battler) {
  ...
  if (this._visualSelectWindow) {
    this._visualSelectWindow.visible = false;
  }
};
...
Window_Help.prototype.drawText = function (text, x, y, maxWidth, align) {
  if ($gameTemp._arrowAllTargets[0]) {
    text = String(Moghunter.bcursor_helpAllEnemies);
  }
  if ($gameTemp._arrowAllTargets[1]) {
    text = String(Moghunter.bcursor_helpAllAllies);
  }
...
// * Initialize
//==============================
var _alias_mog_bcursor_wbca_initialize =
...
Window_BattleActor.prototype.initialize = function (x, y) {
  ...
  if (
    String(Moghunter.bcursor_window) === "true" ||
    (!$gameSystem.isSideView() && !Imported.MOG_BattleHud)
  ) {
    this._window_mode = true;
  }
};
...
// * Process Cursor Move
//==============================
var _alias_mog_bcursor_wbac_processCursorMove =
  Window_BattleActor.prototype.processCursorMove;
...
Window_BattleActor.prototype.update = function () {
  ...
  if (!this._window_mode) {
    this.visible = false;
  }
  if ($gameTemp._arrowTarget[1] != null) {
    this.refresh_touch_selection();
  }
};
...
// * Process Touch
//==============================
var _alias_mog_bcursor_wactor_processTouch =
...
Window_BattleActor.prototype.processTouch = function () {
  if (!this._window_mode && this.active) {
    if (TouchInput.isTriggered() && $gameTemp._arrowAllTargets[1]) {
      this.processOk();
    }
    if (Input.isTriggered("ok") && $gameTemp._arrowAllTargets[1]) {
      this.processOk();
    }
...
Game_Troop.prototype.select = function (activeMember) {
  if ($gameTemp._arrowAllTargets[0]) {
    this.members().forEach(function (member) {
      if (!member.isDead() && !member.isHidden()) {
        member.select();
      }
    });
    return;
...
// * Process Cursor Move
//==============================
var _alias_mog_bcursor_wbeny_processCursorMove =
  Window_BattleEnemy.prototype.processCursorMove;
...
Window_BattleEnemy.prototype.update = function () {
  ...
  if ($gameTemp._arrowTarget[0] != null) {
    this.refresh_touch_selection();
  }
};
...
// * Process Touch
//==============================
var _alias_mog_bcursor_wenmy_processTouch =
  Window_BattleEnemy.prototype.processTouch;
Window_BattleEnemy.prototype.processTouch = function () {
  if (!this._window_mode && this.active) {
    if (TouchInput.isTriggered() && $gameTemp._arrowAllTargets[0]) {
      this.processOk();
    }
    if (Input.isTriggered("ok") && $gameTemp._arrowAllTargets[0]) {
      this.processOk();
    }
    if (TouchInput.isCancelled()) {
      this.processCancel();
