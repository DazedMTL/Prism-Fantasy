...
Moghunter.bresult_treasure_x = Number(
  Moghunter.parameters["Treasure X-Axis"] || 32
);
Moghunter.bresult_treasure_y = Number(
  Moghunter.parameters["Treasure Y-Axis"] || 330
);
...
Moghunter.bresult_par0_x = Number(
  Moghunter.parameters["Parameter Real X-Axis"] || 0
);
Moghunter.bresult_par0_y = Number(
  Moghunter.parameters["Parameter Real Y-Axis"] || 0
);
Moghunter.bresult_par1_x = Number(
  Moghunter.parameters["Parameter 1 X-Axis"] || 550
);
Moghunter.bresult_par1_y = Number(
  Moghunter.parameters["Parameter 1 Y-Axis"] || 140
);
Moghunter.bresult_par2_x = Number(
  Moghunter.parameters["Parameter 2 X-Axis"] || 700
);
Moghunter.bresult_par2_y = Number(
  Moghunter.parameters["Parameter 2 Y-Axis"] || 140
);
Moghunter.bresult_skill_x = Number(
  Moghunter.parameters["New Skill X-Axis"] || 570
);
Moghunter.bresult_skill_y = Number(
  Moghunter.parameters["New Skill Y-Axis"] || 520
);
...
BattleManager.processVictory = function () {
  if (Imported.MOG_BattleCry) {
    ...
    if (actor) {
      SoundManager.selectVoice(actor._v_victory);
    }
  }
...
//==============================
var _mog_bresult_updateWindowPositions =
...
// * Refresh Number
//==============================
BattleResult.prototype.refresh_number = function (
  sprites,
  value,
  img_data,
  x,
...
BattleResult.prototype.update = function () {
  ...
  if (
    this._numbeData[0] === -1 &&
    this._number_img1.isReady() &&
    this._number_img2.isReady()
  ) {
    this.getData();
  }
  if (this._numbeData[0] === -1) {
    return;
  }
  if (this._phaseAnime[1] > 0) {
    this._phaseAnime[1]--;
    return;
  }
  if (this._playME > 0) {
...
BattleResult.prototype.updateLayoutAnimation = function () {
  ...
  if (this._layout.scale.x > 1.0) {
    this._layout.scale.x -= 0.05;
    if (this._layout.scale.x < 1.0) {
      this._layout.scale.x = 1.0;
    }
  } else {
    this.phaseAniClear();
    this._phase = 1;
...
BattleResult.prototype.updateVictAnimation = function () {
  if (this._phaseAnime[0] === 1) {
    if (this._vict.scale.x > 1.0) {
      this._vict.scale.x -= 0.05;
      this._vict.opacity += 15;
      if (this._vict.scale.x <= 1.0) {
        this._vict.scale.x = 1.0;
...
BattleResult.prototype.updateLevelPosition = function () {
  ...
  this._spriteActor.x = this.sprite_move_to(
    this._spriteActor.x,
    Moghunter.bresult_actor_x,
    20
  );
  this._spriteActor.y =
    Graphics.boxHeight - this._spriteActor.height + Moghunter.bresult_actor_y;
  if (this._levelLayout.scale.x > 1.0) {
    this._levelLayout.scale.x -= 0.05;
    if (this._levelLayout.scale.x < 1.0) {
      this._levelLayout.scale.x = 1.0;
    }
  }
