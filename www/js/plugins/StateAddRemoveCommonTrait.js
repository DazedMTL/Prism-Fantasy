...
(function () {
  ...
  DataManager.stateCommonEvent = function (item, stateId, type) {
    ...
    if (
      item._stateCommonId &&
      item._stateCommonId[stateId] !== undefined &&
    ...
    if (item.meta["ステート" + stateId + "付与時コモン"]) {
      item._stateCommonId[stateId]["add"] =
        item.meta["ステート" + stateId + "付与時コモン"];
    } else if (item.meta["AddState" + stateId + "Common"]) {
      item._stateCommonId[stateId]["add"] =
        item.meta["AddState" + stateId + "Common"];
    }
    if (item.meta["ステート" + stateId + "解除時コモン"]) {
      item._stateCommonId[stateId]["remove"] =
        item.meta["ステート" + stateId + "解除時コモン"];
    } else if (item.meta["RemoveState" + stateId + "Common"]) {
      item._stateCommonId[stateId]["remove"] =
        item.meta["RemoveState" + stateId + "Common"];
    }
    if (item.meta["ステート" + stateId + "自然解除時コモン"]) {
      item._stateCommonId[stateId]["autoRemove"] =
        item.meta["ステート" + stateId + "自然解除時コモン"];
    } else if (item.meta["AutoRemoveState" + stateId + "Common"]) {
      item._stateCommonId[stateId]["autoRemove"] =
        item.meta["AutoRemoveState" + stateId + "Common"];
    }
  ...
  var __GInterpreter_setupReservedCommonEvent =
    Game_Interpreter.prototype.setupReservedCommonEvent;
  Game_Interpreter.prototype.setupReservedCommonEvent = function () {
    if (
      !$gameTemp.isCommonEventReserved() &&
      $gameTemp._commonStack &&
      $gameTemp._commonStack.length > 0
    ) {
